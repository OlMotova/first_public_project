...
def check_IP_range_overlap(df_base_IP, first_IP, last_IP):
    ...
    for i in range(len(df_base_IP)):
        ...
        if first_IP <= range_end and range_start <= last_IP:
            text = f'Есть пересечение у введенного диапазона и строки {i + 2} таблица equipment_IP_database.xlsx'
            return False, text
    text = 'все ОК'
    return True, text
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if range1_start <= range2_end and range2_start <= range1_end:
                text = f"Строка файла equipment_IP_database.xlsx {i+2} пересекается со строкой {j+2}"
                return False, text
    text = 'все ОК'
    return True, text
...
def check_ip_serial_difference(df_base_IP):
    ...
    for i in range(len(df_base_IP)):
        ...
        if ip_range_diff != serial_range_diff:
            text = f'Ошибка диапазонов в строке {i + 2}'
            return False, text
    text = 'все ОК'
    return True, text
...
def calculating_button_service (df_base_IP, equipment_name, equipment_type, first_ip, last_ip):
    ...
    if (int(last_IP) - int(first_IP)) < 1:
        ...
        text = "первый введенный IP больше второго"
        return text, False, None, None
    text = "все ок"
    return text, True, first_ZvN, last_ZvN
